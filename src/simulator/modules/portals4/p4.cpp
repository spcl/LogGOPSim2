...
void P4Mod::printStatus() { LogGOPmod::printStatus(); }

bool P4Mod::p4_match(const goalevent &elem, ruq_t *q,
                     ruq_t::iterator *matchedit, uint32_t header_id) {
  ...
  for (ruq_t::iterator iter = q->begin(); iter != q->end(); ++iter) {
    ...
    if (iter->target == ANY_SOURCE || iter->target == elem.target) {
      ...
      if (iter->tag == ANY_TAG || iter->tag == elem.tag) {
        ...
        if (iter->use_once && header_id != 0 && iter->matched_to != 0 &&
            iter->matched_to != header_id) // matched to different header
        {
          continue; //
        }

        if (iter->use_once && header_id != 0 &&
            iter->matched_to == 0) // matched to current header
        {
...
bool P4Mod::p4_match_unexpected(const goalevent &elem, uqhdr_t *q,
                                uqhdr_entry_t *retelem) {

  // std::cout << "UQ size " << q->size() << "\n";

  if (print)
    printf("++ [%i] searching matching unexpected header queue for src %i tag "
           "%i; uq size: %lu\n",
           elem.host, elem.target, elem.tag, q->size());
  for (uqhdr_t::iterator iter = q->begin(); iter != q->end(); ++iter) {

    if (print)
      printf("++ [%i] searching matching unexpected header queue for src %i "
             "tag %i; iter->msg: (%i, %i)\n",
    ...
    if (elem.target == ANY_SOURCE || iter->msg.target == elem.target) {
      if (elem.tag == ANY_TAG || iter->msg.tag == elem.tag) {
        if (print)
          printf("++ [%i] header found in unexepcted queue", elem.host);
        if (retelem)
          *retelem = *iter;
        q->erase(iter);
        return true;
...
int P4Mod::append(goalevent &elem) {
  ...
  if (print)
    printf("[%i] append found. tag: %i; hh: %u; ph: %u; th: %u; mem: %u\n",
  ...
  if ((elem.type == OP_APPEND && nexto[elem.host][elem.proc] <= elem.time) ||
    ...
    if (elem.type != OP_NAPPEND) {
      ...
      // the nextgr is something that depends on external events (incoming
      // messages),
      // the nextgs depends on the operations performed on the NIC by the owner
      // host.
      // TODO: double check. Verify if it is the case to model it in a different
      // way.
    ...
    if ((elem.options & OPT_PRIORITY_LIST) == OPT_PRIORITY_LIST) {
      ...
      while ((i == 0 || (elem.options & OPT_USE_ONCE) != OPT_USE_ONCE) &&
             p4_match_unexpected(elem, &uq_hdr[elem.host], &matched_elem)) {
        if (print)
          printf("-- PTL: match found in UL\n");
        if (matched_elem.msg.type == OP_GETMSG) {
          // TODO: nothing. The P4 spec says that in this case a proper event is
          // generetad. We are not modelling P4 events.
        } else if (matched_elem.msg.type == OP_PUTMSG) {
          // ACCOUNT FOR COPY. PORTALS4 spec said that no copy is performed
          // here. A proper event indicating the start
          // address of the data in the overflow buffer is raised. However, in
          // order to simulate the MPI semantic, we must
          // take in account the copy to the posted buffer (recall that
          // simulated_behavior = f(traces, params)).
          // Assumption: we assume that the copy perfermoed through MM/IO has
          // the same speed of the one perfomerd through DMA.
          // if (elem.type==OP_NAPPEND){ //If it is a triggered append, the copy
          // is charged on the NIC
          //    nextgs[elem.host][elem.nic] += (matched_elem.msg.size)*C;
          //    //latency?
          //    tlviz.add_nicop(elem.host, elem.time+c,
          //    elem.time+c+(matched_elem.msg.size)*C, elem.nic, 0, 0, 0);
          // The copy is charged only if the op is an APPEND. If it is a
          // NAPPEND, thus executed on the NIC, then
          // no copy is perfomed since the NIC cannot copy data from MAINMEMORY
          // TO MAINMEMORY
          if (elem.type == OP_APPEND) {
            // Accounting of unex. msg copy cost disabled to be comparable with
            // the MPI case (where it's not accounted)
            // nexto[elem.host][elem.proc] += (matched_elem.msg.size)*C;
            // //latency?
            sim.visualize(VIS_HOST_DUR,
            ...
            //elem.time+o+(matched_elem.msg.size)*C, elem.proc, 0, 0, 0);
          }
    ...
    // if the ME is for the OVERFLOW_LIST OR is for the PRIORITY LIST but no
    // matching in UL are found OR is for the
    // priority list, matching in UL are found AND the ME is persistent:
    if ((elem.options & OPT_OVERFLOW_LIST) == OPT_OVERFLOW_LIST || i == 0 ||
        (elem.options & OPT_USE_ONCE) != OPT_USE_ONCE) {
      p4_ruqelem_t nelem;
      ...
      for (i = 3; i >= 0; i--) {
        ...
        // printf("arg[%d]=%lu \n",i,elem.arg[i]);
        if (elem.arg[i] != (uint64_t)-1)
          break;
      }

      if (i >= 0) {
        if ((i + 1) * sizeof(uint64_t) > nelem.mem) {
          printf("Error: specify size of shared memory before using it in "
                 "Append\n");
      ...
      if ((elem.options & OPT_OVERFLOW_LIST) == OPT_OVERFLOW_LIST)
...
int P4Mod::nicputget(goalevent &elem) {
  ...
  if (nextgs[elem.host][elem.nic] <= elem.time) {
    if (print)
      printf("[%i] nput/nget op to %i elem.time: %lu\n", elem.host, elem.target,
             elem.time);
    ...
    /* if elem.type==OP_GET, the field "size" contains the size of the data to
    be read, not
    ...
    msg_size = (elem.type==OP_NPUT) ? ((elem.size > S) ? pktsize :  elem.size):
    GET_MSG_SIZE;
    */
    ...
    //elem.time+(msg_size-1)*srate+g, elem.nic);

    // With the GET the operations are triggered on the event PTL_MD_EVENT_REPLY
    // (when we receive the reply)
    if (elem.type != OP_NGET && elem.isParsed)
      TRIGGER_OPS(elem.host, elem.oct, elem.time + DMA_L)
    ...
    */
    ...
    // while (cttable.getNextOperation(elem.host, elem.oct, triggered_op))
...
int P4Mod::putgetmsg(goalevent &elem) {
  ...
  if (print)
    printf(
        "[%i] put/get msg (received). tag: %i; nextgr: %lu; elem.time: %lu\n",
        elem.target, elem.tag, nextgr[elem.target][elem.nic], elem.time);

  uint32_t msg_size =
      (elem.type == OP_PUTMSG) ? elem.size : (uint64_t)GET_MSG_SIZE;

  //###########CHECK BETTER THIS CONDITION!!!
  /*if((earliestfinish = net.query(elem.starttime, elem.time, elem.target,
     elem.host, elem.size, &elem.handle)) <= elem.time &&
      std::max(std::max(nextgr[elem.host][elem.nic],nextc[elem.host][elem.nic]),
     earliestfinish) <= elem.time) &&
      (elem.type!=OP_GETMSG ||
     std::max(std::max(nextgs[elem.host][elem.nic],nextc[elem.host][elem.nic]),
     earliestfinish) <= elem.time)){*/
  if ((nextgr[elem.target][elem.nic] <= elem.time) &&
    ...
    nextgr[elem.host][elem.nic] =
        elem.time + (elem.size - 1) * G +
        std::max(g, c); // TODO: Double check here for "c"
    ...
    //+tara sim.tlviz->add_nicop(elem.host, elem.time+(msg_size-1)*srate,
    //elem.time+(msg_size-1)*srate+c, elem.nic);

    if (print)
      printf("-- msgsize: %u from %i to %i\n", msg_size, elem.target,
             elem.host);
    ...
    if (!nomatch) {
      ...
      if (elem.type == OP_PUTMSG) {
        ...
        // don't charge the copy to the buffer here. Assume that, after the
        // header is received,
        // a byte is copied in the matching buffer as soon as received. This
        // means that the copy
        // is charged in (elem.size-1) * srate, where srate=max(G, C) (we can
        // assume G>C).
        // nextgr[elem.host][elem.nic] += (elem.size-1)*C;
        // tlviz.add_nicop(elem.host, elem.time+(elem.size-1)*srate+c,
        // elem.time+(elem.size-1)*srate+c+(elem.size-1)*C, elem.nic, 0, 0, 0);
        ...
        // The message will be sent after the request will be received
        // ((in_msg_size-1)*C) and evaluated (c)
        ...
        // Insert into the network layer (in order to simulate congestion)
        // net.insert(elem.time, elem.host, elem.target, elem.size,
        // &elem.handle);
      ...
      if (print)
        printf("-- MSG: matched.src: %i; counter: %i; time: %lu;\n",
      ...
      // nextc[elem.host][elem.nic] = elem.time+c+(elem.size-1)*C; //message
      // copy through DMA (assumed)
      // add trasmission from nic to cpu? (visualization) NO! the trasmission is
      // btw the nic and the memory!!!!
    }

  } else {
    //####THIS IS WRONG!!
    // elem.time = std::max(std::max(nextgr[elem.host][elem.nic],
    // nextc[elem.host][elem.nic]), earliestfinish);

    uint64_t gwait = (elem.type == OP_PUTMSG)
                         ? nextgr[elem.target][elem.nic]
                         : std::max(nextgr[elem.target][elem.nic],
                                    nextgs[elem.target][elem.nic]);
    if (print)
      printf("-- no time, reinserting: elem.time: %lu; new time: %lu\n",
             elem.time, gwait);
...
int P4Mod::getreply(goalevent &elem) {
  if (nextgr[elem.target][elem.nic] <= elem.time) {
    if (print)
      printf("[%i] getreply (received). tag: %i; nextgr: %lu; elem.time: %lu\n",
             elem.target, elem.tag, nextgr[elem.target][elem.nic], elem.time);
    ...
    int srate = std::max(G, C);
    // As in the PUTMSG case, we don't have to charge the copy.
    // ((elem.size-1)*C)
    nextgr[elem.host][elem.nic] =
        elem.time + g + (elem.size - 1) * srate; // + (elem.size-1)*C;
    ...
    // elem.target, elem.host,elem.oct, cttable.ct_get_val(elem.host,
    // elem.oct));
    ...
    //elem.time+(elem.size-1)*srate+c, elem.nic);

    // tlviz.add_nicop(elem.host, elem.time+(elem.size-1)*srate, elem.time +
    // (elem.size-1)*srate + (elem.size-1)*C, elem.nic, 0, 0, 0);
...
int P4Mod::tops(goalevent &elem) {
  if (nexto[elem.host][elem.proc] <= elem.time) {
    ...
    if (elem.type == OP_TPUT)
      elem.type = OP_NPUT;
    else if (elem.type == OP_TAPPEND)
      elem.type = OP_NAPPEND;
    else if (elem.type == OP_CTWAIT)
      elem.type = OP_NCTWAIT;
    else if (elem.type == OP_TGET)
      elem.type = OP_NGET;

    if (print)
      printf("[%i] triggered op setup: type: %i; ct: %i; threshold: %lu; time: "
             "%lu\n",
    ...
    if (cttable.ct_get_val(elem.host, elem.ct) >= elem.threshold) {
      // if (elem.type==OP_NCTWAIT && aq.size()>0) printf("CT WAIT: something
      // will overlap with CTWAIT (size: %i)!\n", aq.size());
      sim.reinsertEvent(&elem);
    } else
      cttable.ct_enqueue_op(elem.host, elem.ct, &elem);
