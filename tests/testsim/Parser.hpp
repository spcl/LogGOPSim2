...
*/
	...
	void serialize_mmap(int fd, uint32_t rank, uint32_t num_ranks, uint8_t max_cpu, uint8_t max_nic) {
		...
		if (rank == 0) {
			...
			filesize += (sizeof(char)+sizeof(uint8_t)*2+sizeof(uint32_t)*7+sizeof(uint64_t)/*salvo: */+sizeof(char)+sizeof(uint32_t)*2+sizeof(uint64_t) /* end salvo*/)*allNodes.size(); // nodeinfo
...
class SerializedGraph {
	...
	DeserializedNode get_node_by_offset(uint32_t offset) {
		...
		int SIZEOF_NODE_INFO = sizeof(char) + sizeof(uint64_t) + sizeof(uint32_t)*7 + sizeof(uint8_t)*2/*salvo: */+sizeof(char)+sizeof(uint32_t)*2+sizeof(uint64_t) /* end salvo*/;
