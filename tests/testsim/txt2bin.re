...
/*!re2c
	...
	INT			{ item.target = add_number(s->tok, cursor); if (item.type==tGetOp || item.type == tPutOp) {goto s_25;} else if (item.type==GetOp || item.type==PutOp) {goto s_39;} else { goto s_13;} }
	ANY			{ goto s_err; }
*/
...
/*!re2c
    WS          { goto s_28; }
    INT         { item.threshold = add_number(s->tok, cursor); if (item.type==tAppendOp) {goto s_34;} else {goto s_39;} }
    ANY         { goto s_err; }
*/
...
/*!re2c
    WS          { goto s_33; }
    INT         { item.target = add_number(s->tok, cursor); if (item.type == tAppendOp)  { goto s_25; } else {goto s_39;} }
    ANYRANK     { item.target = std::numeric_limits<uint32_t>::max(); if (item.type == tAppendOp)  { goto s_25; } else {goto s_39;} }
    ANY         { goto s_err; }
*/
