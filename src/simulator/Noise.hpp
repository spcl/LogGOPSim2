  ...
  Noise(gengetopt_args_info *args_info, int p) : p(p) {
    ...
    if(args_info->noise_trace_given) {
      ...
      while(!eof) {
        ...
        //std::cout << ((uint64_t)round(offset*time_mult)) << " " << ((uint64_t)round(duration*time_mult)) << "\n";
        trcnoise.push_back(std::make_pair((uint64_t)round(offset*time_mult), (uint64_t)round(duration*time_mult)));
