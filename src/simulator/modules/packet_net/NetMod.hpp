...
public:
  ...
  HeadPkt(NetMsgEvent ev, uint32_t totpackets)
      : NetMsgEvent(ev), toreceive(totpackets) {
    id = ++idgen;
  }
...
public:
  ...
  DataPkt(uint32_t currenthop, uint32_t destid)
      : currenthop(currenthop), destid(destid) {
    type = NET_PKT;
    header = NULL;
    size = 0;
  }

  DataPkt(HeadPkt *header, uint32_t currenthop, uint32_t size)
      : header(header), currenthop(currenthop), size(size) {
    type = NET_PKT;
  }
...
public:
  HostDataPkt(DataPkt &pkt) : DataPkt(pkt) { type = HOST_DATA_PKT; }

  HostDataPkt(HostDataPkt &pkt, int _type) : HostDataPkt(pkt) { type = _type; }
};
...
public:
  NetMod(Simulator &sim, bool deliver_single_packets = false)
    ...
    char *mapping_file = sim.args_info.network_mapping_given
                             ? sim.args_info.network_mapping_arg
                             : NULL;
  ...
  int ingress(NetMsgEvent &msg) {
    ...
    if (print)
      printf("[NET] ingress packet: time: %lu\n", msg.time);
  ...
  int _forward(DataPkt &msg) {
    ...
    if (print) {
      printf("[NET %s] forwarding packet: %i->%i(%i); msg.time: %lu; \
                    nexto[%i]: %lu\n",
             topology->get_nodename(sw).c_str(), msg.header->source, msg.destid,
             msg.id, msg.time, sw, nexto[sw]);
    }

    /* check if there is time */
    if (nexto[sw] > msg.time) { /* no time */
      if (print)
        printf("\tno time; nexto[%i]: %lu; msgtime: %lu\n", sw, nexto[sw],
               msg.time);
    ...
    if (msg.is_credit) {
      ...
      if (!buffer[sw][topology->next_port(sw, msg.destid)].empty()) {
        ...
        DataPkt *toforward =
            buffer[sw][topology->next_port(sw, msg.destid)].front();
    ...
    if (!endswitch && credits[sw][topology->next_port(sw, msg.destid)] == 0) {
      if (print) {
        printf("\tno credits: switch: %i; port: %i; msg.time: %lu; \
                        new time: %lu\n",
               sw, topology->next_port(sw, msg.destid), msg.time,
    ...
    /* msg.lasthop==msg.currenthop only if this is the source host
       (hence the message is already buffered and we don't have to increase the
       credits) */
