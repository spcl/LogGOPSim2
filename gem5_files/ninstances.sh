...
do
    ...
    for j in `seq 1 $cpus`;
    do
        sed -e "s/SYSNAME/system$i/g" $cputemplate | sed -e "s/CPUNAME/cpu$j/g" | sed -e "s/P1/$((membusport+1))/g" | sed -e "s/P2/$((membusport+2))/g" | sed -e "s/P3/$((membusport+3))/g" | sed -e "s/P4/$((membusport+4))/g" | sed -e "s/CPUID/$((j-1))/g" 
