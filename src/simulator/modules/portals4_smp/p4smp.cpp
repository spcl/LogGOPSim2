...
int P4SMPMod::recvpkt(HostDataPkt& pkt){
    ...
    /* this case has to be taken into accout now since new operations can be generates by SMP handlers */
    if (host>=nextgr.size()) { printf("Error: Host %i does not exist (check goal file)\n", host); exit(-1); }
    ...
    if (print) printf("[%i] receivepkt. id: %i; tag: %i; nextgr: %lu; elem.time: %lu; pkt.time: %lu; pkt.size: %u; type: %u\n", host, pkt.id, elem.tag, nextgr[host][elem.nic], elem.time, pkt.time, pkt.size, elem.type);
    ...
    if((nextgr[host][elem.nic] <= pkt.time)){
        ...
        if(pkt.isTail())   
            nextgr[host][elem.nic]+=std::max(g,c+pkt.id*c_packet) - (c+pkt.id*c_packet);

    
        sim.visualize( VIS_HOST_FLOW ,
            new HostComplexFlowVisEvent("Name:add_transmission",elem.target, host,SET_NIC(0),SET_NIC(0),  pkt.starttime, pkt.time, pkt.size, G)
        );
        //+tara sim.tlviz->add_transmission(elem.target, host, pkt.starttime, pkt.time, pkt.size, G);
        sim.visualize( VIS_HOST_DUR ,
            new HostDurationVisEvent("Name:add_nicop",elem.host,SET_NIC(elem.nic),pkt.time, pkt.time+(pkt.size-1)*G)
            );
        //+tara sim.tlviz->add_nicop(elem.host, pkt.time, pkt.time+(pkt.size-1)*G, elem.nic);
        sim.visualize( VIS_HOST_DUR ,
            new HostDurationVisEvent("Name:add_nicop",elem.host,SET_NIC(elem.nic),pkt.time+(pkt.size-1)*G, nextgr[host][elem.nic])
            );
        ...
        /* reinsert packet */
        if (print) printf("-- no time: reinserting pkt. pkt.time: %lu; nextgr[%i]: %lu;\n", pkt.time, host, nextgr[host][elem.nic]);
...
int P4SMPMod::processHandlers(MatchedHostDataPkt& pkt){
    ...
    while (pkt.hasHandlers()){
        ...
        assert(!pkt.isCurrentHandlerSuspended() || (pkt.isCurrentHandlerSuspended() && res!=NO_CONTEXT_AVAIL && res!=NO_HPU_AVAIL));

        if (res == NO_CONTEXT_AVAIL){   
            if (print) printf("Event in waiting queue. time: %lu; host[%i]\n", current, host);
            return 0;
        }
    ...
    if (fullpacket){
        //printf("trigger ops %lu, %lu, %lu \n", pkt.matched.src, pkt.matched.counter, current);
        if (elem.type==OP_GETMSG){
        ...
        sim.visualize( VIS_HOST_DUR ,
            new HostDurationVisEvent("Name:add_nicop",elem.host,SET_NIC(elem.nic),current, current+DMA_L)
        );
        ...
        if (pkt.matched.use_once) {   
            if(*pkt.matched.pending) reset_matching_entry(host, pkt.header->id);
            else erase_matching_entry(host, pkt.header->id);
        }
