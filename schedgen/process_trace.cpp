...
// for sPIN paper!
static const int eager_limit = 32768;    // eager limit for sPIN
...
#if 0
  // this here supports the old format "0" vs. "00" etc.
  if(substr == NULL) { std::cerr << "tracefile-name did not contain '-0' - exiting\n"; throw(130); }
  *(substr+1) = '\0'; // overwrite "0" with \n - now we have two strings in mask 
                      // - the beginning at offset 0 and the end at offset *(substr+1)
  char *end = substr+2;
  //printf(">>>>> %s\n", mask);
  //printf(">>>>> %s\n", (substr+2));
  snprintf(out, 4096, "%s%i%s", in, host, end);
  //printf(">>>>> %s\n", out);
#endif
...
void process_trace(gengetopt_args_info *args_info) {
  ...
  while(1) {
    ...
    //std::cout << "attempting file: "<<buffer<< " ("<<host<<")"<<std::endl;
  ...
  int copy_overhead_multiplier = args_info->traces_copyemu_arg; // this is emulating the copy costs at the target CPU (in % of the network bandwidth)
                                                                // we implement it by increasing the message size for eager messages 
                                                                // proportionally, and for rendezvous, we add it only to the eager limit size
  ...
  if(copy_overhead_multiplier) std::cout << "with copy overhead multiplier " << copy_overhead_multiplier << "\n";
      ...
      while(!found_eof) {
        ...
        if(args_info->traces_nop2p_given) {
          ...
          if(pars.match(&e_recv, funchash, line, &match)) {
            ...
            if(copy_overhead_multiplier) {
              if(count*size <= eager_limit)
                count += ceil((float)count*(float)copy_overhead_multiplier/100.0); // will round up!
              else
                count += ceil((float)eager_limit*(float)copy_overhead_multiplier/100.0); 
            }
          ...
          if(pars.match(&e_send, funchash, line, &match)) {
            ...
            if(copy_overhead_multiplier) {
              if(count*size <= eager_limit)
                ...
                count += ceil((float)eager_limit/size*(float)copy_overhead_multiplier/100.0); 
            }
          ...
          if(pars.match(&e_wait, funchash, line, &match)) {
            ...
            //if(copy_overhead_multiplier) goto endloop;
          ...
          if(pars.match(&e_sendrecv, funchash, line, &match)) {
            ...
            if(copy_overhead_multiplier) {
              if(scount*ssize <= eager_limit) 
                scount += ceil((float)scount*(float)copy_overhead_multiplier/100.0); // will round up!
              else
                scount += ceil((float)eager_limit/rsize*(float)copy_overhead_multiplier/100.0); 
              if(rcount*rsize <= eager_limit) 
                rcount += ceil((float)scount*(float)copy_overhead_multiplier/100.0); // will round up!
              else
                rcount += ceil((float)eager_limit/rsize*(float)copy_overhead_multiplier/100.0); 
            }
...
endloop:
        ...
        if(args_info->traces_nops_arg && nops == args_info->traces_nops_arg) {
          std::cout << "processed " << nops << " operations, finished host " << host << "\n";
          if(startpos.size()) {
