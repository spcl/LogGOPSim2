...
class TopoGraph {
    ...
    public:
        ...
        TopoGraph(char* filename, char * mapping_filename, bool print): last_free_rank(0) {
            /* This needs to be before the parsing of dotfile */
            if (mapping_filename!=NULL) this->parse_mapping_file(mapping_filename);
        ...
        void parse_dotfile_agraph(char* filename) {
            ...
            while (node != NULL) {
                ...
                if (nodetype == NODE_TYPE_HOST) {
                    ...
                    if (hostname!=NULL){
                        std::map<std::string, uint32_t>::iterator it = nodename2rank.find(hostname);
                        
                        if (it == nodename2rank.end()) rank = get_free_rank();
                        else rank = it->second;
                    } else { rank = get_free_rank(); }
