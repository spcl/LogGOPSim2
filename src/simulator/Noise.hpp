...
public:
  Noise(gengetopt_args_info *args_info, int p) : p(p) {
    ...
    if (args_info->noise_trace_given) {
      ...
      if (!trace.is_open()) {
        std::cerr << "couldn't read noise trace file: "
                  << args_info->noise_trace_arg << " - exiting\n";
        throw(10);
      }

      time_mult = args_info->noise_time_mult_arg;
      bool eof = false;
      int line = 0;
      while (!eof) {
        line++;
        ...
        // std::cout << ((uint64_t)round(offset*time_mult)) << " " <<
        // ((uint64_t)round(duration*time_mult)) << "\n";
        trcnoise.push_back(
            std::make_pair((uint64_t)round(offset * time_mult),
                           (uint64_t)round(duration * time_mult)));

        eof = trace.eof();
      }

      if (((trcnoise.end() - 1)->first - trcnoise.begin()->first) >
          (double)std::numeric_limits<uint64_t>::max()) {
        std::cerr << " the length of the noise-trace ("
                  << (trcnoise.end() - 1)->first - trcnoise.begin()->first
                  << " ns) is can not be saved in 'uint64_t' (max: "
                  << (double)std::numeric_limits<uint64_t>::max()
                  << ") - exiting\n";
        throw(11);
      }
      ...
      std::cout << "Noisegen: read " << trcnoise.size()
                << " noise events spanning " << trctime / 1e9 << "s ";
  ...
  inline btime_t get_noise(int r, btime_t starttime /* in ns */,
                           btime_t endtime /* in ns */) {
    btime_t noise = 0;

    // if we have trace data
    if (trcnoise.size()) {

      // printf("noise: start: %lu; stop: %lu\n", starttime, endtime);
      ...
      btime_t trcstart =
          (starttime + ranktime[r]) % (trcnoise.back().first - oplength);
      ...
      if (trcstart > trcnoise[0].first) {
        ...
        while (!( // we loop until we have:
            (trcnoise[pos].first <=
             trcstart) && // pos is smaller or equal than trcstart
            (trcnoise[pos + 1].first >
             trcstart) // pos+1 is larger than trcstart
            )) {
          if (trcnoise[pos].first > trcstart)
            pos--;
          else
            pos++;
        };
      ...
      // if(noise > 100000) std::cout << trcnoise[pos].first << " " << trcstart
      // << " " << trcnoise[pos+1].first << "\n";
      ...
      while (end > trcnoise[pos + 1].first) {
        ...
        /*if(noise > 100000) {
          std::cout << "inner " << trcnoise[pos].first << " pos: " << pos << "
        start-end: " << trcstart << "-" << end << " end: " << end << " noise: "
        << noise << " trctime: " << trctime << "\n";
        return 0;}*/
      ...
      // if (noise > 100000) std::cout << "injected " << noise << " ns noise in
      // " << endtime-starttime << "ns\n";
      if (p <= max_report)
        injected_noise[r] += noise;
