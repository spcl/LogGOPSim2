...
int DMAmod::handleDMARequest(DMAEvent& elem){
    ...
    // printf("[%i] found send to %i (size: %u)- t: %lu (CPU: %i) - nexto: %lu - nextgs: %lu\n", elem.host,  elem.size, elem.time, nextos[elem.host], nextgs[elem.host]);

    if(nextgs[elem.host] <= elem.time) { // local g available!
       
        btime_t starttime = elem.time;
        if (print) printf("[%i] DMA servicing event @%lu\n", elem.host, elem.time);
        ...
        if (elem.type==DMA_WRITE){
            ...
            /* the data has to pay lantecy of the request + data transfer + latency of response */
            if(contention)
       ...
       sim.visualize( VIS_HOST_DUR ,
        ...
        if(elem.isBlocking){ // blocking 
            ...
            if(it!=wq[host].end()){
                ...
                if (print) printf("-- wait already received\n");
                ...
                if (print) printf("-- still no wait, put elem in wait queue\n");

                //add it to unexpected queue. It means that read completed earlier than blocking wait 
                uq[host].push_back(elem);
            }   
        }
    } else { // local o,g unavailable - retry later
        if (print) printf("-- send local g not available -- reinserting \n");
...
int DMAmod::wait(DMAEvent& elem){
    ...
    if(it!=uq[host].end()){
       // read completed earlier than blocking wait 
        ...
        if (print) printf("[%i] DMA got wait (expected) @%lu\n", elem.host, elem.time);
        elem.time = (*it).time;
        if (print) printf("-- reinserting at @%lu\n", elem.time);
        sim.addEvent(elem.extractOriginatingEvent());
        uq[host].erase(it);
    }else{
        if (print) printf("[%i] DMA got wait (UNexpected) @%lu\n", elem.host, elem.time);

        //add it to waiting queue. It means that  blocking wait is earlier than read completed
