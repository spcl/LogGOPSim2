	...
	if (myfile.is_open()) {
		...
		while (!myfile.eof()) {
			...
			if (boost::regex_match(line.c_str(), matches, osend)) {
				...
				if ((interval==false) or ((atoi(starts.c_str()) >= args_info.starttime_arg) && (atoi(ends.c_str()) < args_info.endtime_arg))) {
					TLViz.add_ohpu(atoi(ranks.c_str()), atoi(starts.c_str())-args_info.starttime_arg, atoi(ends.c_str())-args_info.starttime_arg, atoi(hpus.c_str()),
					atof(reds.c_str()), atof(greens.c_str()), atof(blues.c_str()) );
					if (maxtime < atoi(ends.c_str())) maxtime = atoi(ends.c_str());
					if (maxhpu < atoi(hpus.c_str())) maxhpu = atoi(hpus.c_str());
				}
	
            }
            else if (boost::regex_match(line.c_str(), matches, odma)) {
                
				std::string ranks = matches[1];
				std::string starts = matches[2];
				std::string ends = matches[3];
				std::string reds = matches[4];
				std::string greens = matches[5];
				std::string blues = matches[6];

				if ((interval==false) or ((atoi(starts.c_str()) >= args_info.starttime_arg) && (atoi(ends.c_str()) < args_info.endtime_arg))) {
					TLViz.add_odma(atoi(ranks.c_str()), atoi(starts.c_str())-args_info.starttime_arg, atoi(ends.c_str())-args_info.starttime_arg,
					atof(reds.c_str()), atof(greens.c_str()), atof(blues.c_str()) );
					if (maxtime < atoi(ends.c_str())) maxtime = atoi(ends.c_str());
				}
            ...
            else if (boost::regex_match(line.c_str(), matches, nicop)) {

				std::string ranks = matches[1];
				std::string nics = matches[2];
				std::string starts = matches[3];
				std::string ends = matches[4];
				std::string reds = matches[5];
				std::string greens = matches[6];
				...
				if ((interval==false) or ((atoi(starts.c_str()) >= args_info.starttime_arg) && (atoi(ends.c_str()) < args_info.endtime_arg))) {
					TLViz.add_nicop(atoi(ranks.c_str()), atoi(starts.c_str())-args_info.starttime_arg , atoi(ends.c_str())-args_info.starttime_arg, atoi(nics.c_str()),
					atof(reds.c_str()), atof(greens.c_str()), atof(blues.c_str()) );
					if (maxtime < atoi(ends.c_str())) maxtime = atoi(ends.c_str());
					//if (maxcpu < atoi(cpus.c_str())) maxcpu = atoi(cpus.c_str());
				}
		...
		TLViz.init_graph(rank_num, maxcpu+1, maxhpu+1, 800, 800, args_info.outputfile_arg);
