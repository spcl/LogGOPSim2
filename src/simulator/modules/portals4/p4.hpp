...
#define TRIGGER_OPS(HOST, CT, TIME)  \
    ...
    while (cttable.getNextOperation(HOST, CT, &triggered_op)) {\
        ...
        triggered_op->time = std::max(triggered_op->time, TIME + (btime_t)(triggered_op->type==OP_NCTWAIT ? 0 : DMA_L) );\
...
class ptl_counter{
    public:
    uint64_t value;
    std::priority_queue<goalevent*, std::vector<goalevent*>, triggerd_compare_func> triggered;
    ...
    bool getNextOperation(int host, int handle, goalevent **t_op){
       ...
       // if (!ct->triggered.empty()) printf("ct: value: %i; next threshold: %i\n", ct->value, ct->triggered.top().threshold);
        if (!ct->triggered.empty() && ct->value == ct->triggered.top()->threshold){
            ...
            *t_op = (goalevent *) ct->triggered.top(); ct->triggered.pop();
            return true;
        }
        return false;
    }
...
protected:
    ...
    std::vector<uqhdr_t> uq_hdr; //contains info about unexpected messages matching
    std::vector<ruq_t> rq, uq; // receive queue, unexpected queue

    

public:
    P4Mod(Simulator& sim, Parser& parser, bool simplenet=true): LogGOPmod(sim, parser, simplenet) {
    ...
    //bool p4_match(const goalevent &elem, P4SMPMod::ruq_t *q, p4_ruqelem_t * retelem=NULL, bool remove=true, ruq_t::iterator * matchedit = NULL);
    bool p4_match(const goalevent &elem, P4Mod::ruq_t *q, ruq_t::iterator * matchedit, uint32_t header_id=0);
        
    bool p4_match_unexpected(const goalevent &elem, uqhdr_t *q, uqhdr_entry_t *retelem=NULL);
