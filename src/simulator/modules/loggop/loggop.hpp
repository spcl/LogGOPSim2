...
public:
  ...
  NetMsgEvent(simEvent *g, uint32_t source, uint32_t dest, uint32_t size,
              btime_t time)
      : payload(g), source(source), dest(dest), size(size) {
    type = NET_MSG;
    this->time = time;
    g->keepalive = true;
  }

  NetMsgEvent(LogGPBaseEvent *g, uint32_t size, btime_t time)
      : NetMsgEvent(g, g->host, g->target, size, time) {
    ;
  }

  NetMsgEvent(LogGPBaseEvent *g, btime_t time)
      : NetMsgEvent(g, g->host, g->target, g->size, time) {
    ;
  }
...
public:
  LogGOPmod(Simulator &sim, Parser &parser, bool simplenet = true)
      : sim(sim), parser(parser), simplenet(simplenet) {
    ...
    printf("LogGOPMod: hosts %i (%i CPUs, %i NICs); L=%i, o=%i g=%i, G=%i, "
           "O=%i, P=%i, S=%u\n",
  ...
  ~LogGOPmod() {
    ...
    if (print) {
      for (uint32_t i = 0; i < p; i++) {
        if (!rq[i].empty())
          printf("Host %u: RQ **NOT** EMPTY! (size: %lu)\n", i, rq[i].size());
        if (!uq[i].empty())
          printf("Host %u: UQ **NOT** EMPTY! (size: %lu)\n", i, rq[i].size());
      }
    }
  ...
  virtual int registerHandlers(Simulator &sim) {
    ...
    if (simplenet)
      sim.addEventHandler(this, NET_MSG, LogGOPmod::dispatch);
