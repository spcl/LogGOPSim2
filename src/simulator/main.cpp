...
int main(int argc, char *argv[]) {
  ...
  if (gem5) {
    printf(
        "Warning: configured without GEM5 support. Ignoring gem5 conf file.\n");
    gem5 = false;
  }
  lmod = new P4Mod(sim, parser, simplenet);
  ...
  if (args_info.vizfile_given) {
    TimelineVisualization *tlviz = new TimelineVisualization(
        args_info.vizfile_arg, parser.schedules.size());
