...
#include "clientlib/lgslib.h"

#include <map>

/* Warning: here we make a strong assumption on the config.ini: cpus appear in
 * system #id order. */
...
int gem5Mod::dispatch(simModule *mod, simEvent *_elem) {
...
gem5Mod::gem5Mod(Simulator &sim, uint32_t ranks) : sim(sim), p(ranks) {
  ...
  /*for(std::list<SimObject*>::iterator it =
  config_manager->objectsInOrder.begin(); it !=
  config_manager->objectsInOrder.end(); ++it) {
  ...
  for (std::map<std::string, SimObject *>::iterator it =
           config_manager->objectsByName.begin();
       it != config_manager->objectsByName.end(); ++it) {
  ...
  if (dummycaches.size() != cpus.size()){
    printf("Error: Only %lu DummyCache modules found! Expected: %lu. \
        Check the gem5 conf file!\n", dummycaches.size(), cpus.size());
    exit(1); 
  }
  ...
  for (uint32_t i = 0; i < dummycaches.size(); i++) {
    ...
    if (i % hpus != 0)
      dummycaches[i]->setParent(dummycaches[(i / hpus) * hpus]);
    dummycaches[i]->intercept(DATA, 33568, false, i % hpus != 0);
  }

  for (uint32_t i = 0; i < systems; i++) {
    /* suspend all the hpus but 0 (we need just one cpu to perform the memory
     * binding) */
  ...
  while (!curEventQueue()->empty() /*&& suspended < mems.size()*/) {
    ...
    /* TODO: there could be a way to get the system index of the last serviced
     * event */
    for (uint32_t i = 0; i < systems; i++) {
      ...
      if (hpuid < hpus && DUMMYCACHE(i, hpuid)->monitor(CTRL)) {
        ...
        if (!DUMMYCACHE(i, hpuid)->allIntercepted()) {
          printf(
              "## Warning: DummyCache(%i %i) suspending context before "
              "intercepting\n",
              i, hpuid);
        }
...
int gem5Mod::executeHandler(gem5SimRequest &elem) {
  ...
  if (!elem.isSuspended()) { /* in this case we have to seek resources */
    ...
    if (nexthpu[host].empty()) {
      if (print) {
        printf(
            "--no context available; time: %lu; host[%i]; Keeping event in "
            "th gem5mod waiting queue\n",
            time, host);
      }
    ...
    /* Check if the hpu is available at this time. If not, we now have an
     * estimate. */
    if (hpu.time > time) { /* not available */
      if (print)
     ...
     elem.time = hpu.time;
      ...
      /* reinsert the event in the active queue */
    ...
    // printf("copying data (%i %i): %p\n", host, hpu.hid, DUMMYCACHE(host,
    // hpu.hid)->getPtr(DATA));
    /* Copy message */
    if (elem.hasCopyFun()){
      elem.copy_fun(elem.copy_fun_ref, DUMMYCACHE(host, hpu.hid)->getPtr(DATA), SHARED_DATA_SIZE);
      //elem.copyHandlerData(DUMMYCACHE(host, hpu.hid)->getPtr(DATA));
    }
  ...
  if (print)
    printf("ExecuteHandler host: [%d]; hpu.hid: [%d]; hpus: %i (%p)\n", host,
           hpu.hid, hpus, HPU(host, hpu.hid));
  HPU(host, hpu.hid)->activateContext(0);

  Tick t = curTick();

  while (!curEventQueue()->empty() &&
         !DUMMYCACHE(host, hpu.hid)->monitor(CTRL)) {
    curEventQueue()->serviceOne();
    ...
    if (DUMMYCACHE(host, hpu.hid)->monitor(SIMCALL)) {
      ...
      uint32_t syscall_num =
          *((uint32_t *)DUMMYCACHE(host, hpu.hid)->getPtr(SIMCALL_NUM));
      ...
      btime_t simcall_time = hpu.time + htot;
      bool suspend;
      btime_t simcall_latency = simcall(elem, syscall_num, syscall_data, simcall_time, &suspend);
