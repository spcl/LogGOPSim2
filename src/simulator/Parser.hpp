    ...
    */
    ...
    for (std::vector<Node *>::iterator it = allNodes.begin();
      ...
      if ((**it).Type == OPTYPE_SEND)
        ...
        strcpy(typestr, "Unkn");
      fprintf(fd, "%i [label=\"<f0> Type: %s | <f1> Peer: %i | <f2> Size: %lu "
                  "| <f3> Tag: %i | <f4> Proc: %i | <f5> Nic: %i \"]\n",
  ...
  /*
      void serialize(FILE* fd, uint32_t rank, uint32_t num_ranks) {
          ...
          if (rank == 0) {
              ...
              fseek(fd, sizeof(uint64_t)*2*num_ranks + sizeof(uint32_t),
     SEEK_SET);
          }
          else {
              // jump to the end of the file
              fseek(fd, sizeof(uint32_t) + (2*(rank-1)+1)*sizeof(uint64_t),
     SEEK_SET);
              uint64_t eos;
          ...
          long pos_in_appendix = start +
                                 sizeof(uint32_t)*2 + // for Nodecount and
     Independent Actions count
                                 sizeof(uint32_t)*RootNodes.size() + // for
     indp. actions offsets
                                 (sizeof(char) + sizeof(uint8_t)*2 +
     ...
     nodeinfo
          ...
          fwrite(&buf32, sizeof(uint32_t), 1, fd); // number of independent
     actions
          for (std::vector<Node*>::iterator it = RootNodes.begin(); it !=
     RootNodes.end(); it++) {
              buf32 = (**it).offset;
              fwrite(&buf32, sizeof(uint32_t), 1, fd); // offset of independent
     action
          }
     ...
     allNodes.end(); it++) {
              ...
              // write the actual operation info - this is a bit complicated
     thanks to padding that the compiler might do...
              ...
              fwrite( &buf32, sizeof(uint32_t), 1, fd ); // number of ops
     depending on this node

              if ((**it).DependOnMe.size() > 0) {
                  fwrite( &num_in_appendix, sizeof(uint32_t), 1, fd ); // start
     of dependant offsets in appendix
              }
              else {
                  uint32_t b = 999;
     ...
     dit != (**it).DependOnMe.end(); dit++) {
              ...
              fwrite( &buf32, sizeof(uint32_t), 1, fd ); // number of ops
     start-depending on this node
              if ((**it).StartDependOnMe.size() > 0) {
                  fwrite( &num_in_appendix, sizeof(uint32_t), 1, fd ); // start
     of start-dependant offsets in appendix
              }
              else {
                  uint32_t b = -1;
     ...
     (**it).StartDependOnMe.begin(); dit != (**it).StartDependOnMe.end(); dit++)
     {
                  fwrite( &((**dit).offset), sizeof(uint32_t), 1, fd );
              }
              // adjust number of elements and position in appendix
          ...
          // write jumptable info: the schedule started at start and ended at
     pos_in_appendix
          fseek(fd, 0, SEEK_SET);
  ...
  */
  ...
  void serialize_mmap(int fd, uint32_t rank, uint32_t num_ranks,
    ...
    if (rank == 0) {
      ...
      // filesize +=
      // (sizeof(char)+sizeof(uint8_t)*2+sizeof(uint32_t)*7+sizeof(uint64_t))*allNodes.size();
      // // nodeinfo
      filesize += (sizeof(char) + sizeof(uint8_t) * 2 + sizeof(uint32_t) * 7 +
                   sizeof(uint64_t) /*salvo: */ + sizeof(char) +
                   sizeof(uint32_t) * 6 + sizeof(uint64_t) * 5 /* end salvo*/) *
      ...
      *((uint64_t *)mapping_start) = (uint64_t)MAGIC_COOKIE;
      ...
      start_rankdata =
          ...
          sizeof(uint64_t) * 2 *
              num_ranks; // our rankdata starts right after the jumptable
    ...
    *((uint32_t *)pos) = (uint32_t)RootNodes.size();
    ...
    for (std::vector<Node *>::iterator it = allNodes.begin();
      ...
      *((uint64_t *)pos) = (**it).Size;
      pos += sizeof(uint64_t); // size of the data transfer / length of the
                               // local calculation
      *((uint32_t *)pos) = (**it).Tag;
      ...
      *((uint32_t *)pos) = (**it).DependOnMe.size();
      pos += sizeof(uint32_t); // number of actions that depend on this actions
                               // termination
      ...
      *((uint32_t *)pos) = (**it).StartDependOnMe.size();
      pos += sizeof(
          uint32_t); // number of actions that depend on this actions start
      *((uint32_t *)pos) = num_in_appendix;
      pos += sizeof(
          uint32_t); // start index of start-dependent actions (in appendix)
      num_in_appendix += (**it).StartDependOnMe.size();
      // printf("[salvo][serializing] option field is in pos: %ld\n", pos -
      // mapping_start);
      *((char *)pos) = (**it).options;
      pos += sizeof(char); // options
      *((uint32_t *)pos) = (**it).ct;
      pos += sizeof(uint32_t); // counter id
      *((uint64_t *)pos) = (**it).threshold;
      pos += sizeof(uint64_t); // threshold
      *((uint32_t *)pos) = (**it).oct;
      pos += sizeof(uint32_t); // owned counter
      ...
      *((uint64_t *)pos) = (**it).arg[0];
      pos += sizeof(uint64_t);
      *((uint64_t *)pos) = (**it).arg[1];
      pos += sizeof(uint64_t);
      *((uint64_t *)pos) = (**it).arg[2];
      pos += sizeof(uint64_t);
      *((uint64_t *)pos) = (**it).arg[3];
      pos += sizeof(uint64_t);
    }
    ...
    *((uint8_t *)(mapping_start + sizeof(uint32_t))) =
        max_cpu; // minimal number of cpu required to simulate this schedule
    *((uint8_t *)(mapping_start + sizeof(uint32_t) + sizeof(uint8_t))) =
        max_nic; // minimal number of nics required to simulate this schedule
    ...
    *((uint64_t *)(mapping_start + sizeof(uint32_t) + sizeof(uint8_t) * 2 +
    ...
    int r =
        munmap(mapping_start - sizeof(uint64_t), (size_t)(pos - mapping_start));
    assert(r == 0);
  }
};
...
private:
  ...
  void add_root_nodes() {
    ...
    for (uint32_t cnt = 0; cnt < num_root_nodes; cnt++) {
      ...
      uint32_t offset =
          (uint32_t) * ((uint32_t *)(mapping_start + sizeof(uint32_t) * 2 +
                                     cnt * sizeof(uint32_t)));
  ...
  DeserializedNode get_node_by_offset(uint32_t offset, uint64_t time = 0) {
    ...
    int SIZEOF_NODE_INFO =
        sizeof(char) + sizeof(uint64_t) + sizeof(uint32_t) * 7 +
        sizeof(uint8_t) * 2 /*salvo: */ + sizeof(char) + sizeof(uint32_t) * 6 +
        sizeof(uint64_t) * 5 /* end salvo*/;
    ...
    N.Peer = (uint32_t) * ((uint32_t *)(start_of_node + sizeof(uint32_t) +
    ...
    // (start_of_node + sizeof(char) + sizeof(uint64_t) + sizeof(uint32_t)*7 +
    // sizeof(uint8_t)*2) - _map_start);
    N.options =
        (char)*((char *)(start_of_node + sizeof(char) + sizeof(uint64_t) +
                         sizeof(uint32_t) * 7 + sizeof(uint8_t) * 2));
    N.ct = (uint32_t) *
           ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) +
                         sizeof(uint32_t) * 7 + sizeof(uint8_t) * 2));
    N.threshold =
        (uint64_t) *
        ((uint64_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) +
                      sizeof(uint32_t) * 8 + sizeof(uint8_t) * 2));
    N.oct =
        (uint32_t) *
        ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 8 + sizeof(uint8_t) * 2));
    // printf("[salvo][deserializing] hh field should be in pos: %ld\n",
    // (start_of_node + sizeof(char)*2 + sizeof(uint64_t)*2 + sizeof(uint32_t)*9
    // + sizeof(uint8_t)*2) - _map_start);

    N.hh =
        (uint32_t) *
        ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 9 + sizeof(uint8_t) * 2));
    N.ph =
        (uint32_t) *
        ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 10 + sizeof(uint8_t) * 2));
    N.ch =
        (uint32_t) *
        ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 11 + sizeof(uint8_t) * 2));
    N.mem =
        (uint32_t) *
        ((uint32_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 12 + sizeof(uint8_t) * 2));

    N.arg[0] =
        (uint64_t) *
        ((uint64_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 2 +
                      sizeof(uint32_t) * 13 + sizeof(uint8_t) * 2));
    N.arg[1] =
        (uint64_t) *
        ((uint64_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 3 +
                      sizeof(uint32_t) * 13 + sizeof(uint8_t) * 2));
    N.arg[2] =
        (uint64_t) *
        ((uint64_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 4 +
                      sizeof(uint32_t) * 13 + sizeof(uint8_t) * 2));
    N.arg[3] =
        (uint64_t) *
        ((uint64_t *)(start_of_node + sizeof(char) * 2 + sizeof(uint64_t) * 5 +
                      sizeof(uint32_t) * 13 + sizeof(uint8_t) * 2));

    // printf("[Parser] deserializing: type: %c; peer: %u; deps from: %u; dep
    // to: %u\n", N.Type, N.Peer, N.DependenciesCnt, num_deps);

    // printf("yyy 3\n");
    // printf("yyy 3 start of apdx = mapping start + %i\n", sizeof(uint32_t)*2 +
    // sizeof(uint32_t)*num_root_nodes + SIZEOF_NODE_INFO*num_nodes);
    ...
    for (uint32_t cnt = 0; cnt < num_deps; cnt++) {
      // printf("yyy 3.5 (%i, %i)\n", num_deps, cnt);
      // printf("yyy (start of appendix: %i, deps startoffset in apdx %i, cnt
      // %i)\n", start_of_apdx, deps_startoffset_in_apdx, cnt);
      uint32_t depnode =
          (uint32_t) *
          ((uint32_t *)(start_of_apdx +
                        (deps_startoffset_in_apdx + cnt) * sizeof(uint32_t)));
    ...
    for (uint32_t cnt = 0; cnt < num_startdeps; cnt++) {
      N.StartDependOnMe.push_back(
          (uint32_t) * ((uint32_t *)(start_of_apdx +
                                     (startdeps_startoffset_in_apdx + cnt) *
                                         sizeof(uint32_t))));
    }
    // printf("yyy 5\n");
...
public:
    ...
    */
    ...
    for (std::vector<DeserializedNode>::iterator it = allNodes.begin();
      ...
      if ((*it).Type == OPTYPE_SEND)
        ...
        strcpy(typestr, "Unkn");
      fprintf(fd, "%i [label=\"<f0> Type: %s | <f1> Peer: %i | <f2> Size: %llu "
                  "| <f3> Tag: %i | <f4> Proc: %i | <f5> Nic: %i \"]\n",
              (*it).offset, typestr, (*it).Peer, (unsigned long long)(*it).Size,
              (*it).Tag, (*it).Proc, (*it).Nic);
    }
  ...
  SerializedGraph(char *map_start, size_t map_length, uint32_t rank) {
    ...
    ssched =
        *((uint64_t *)
              tmp); // jumping over num_schedules + max_cpu/max_nic + jumptable
  ...
  void MarkNodeAsStarted_DSN(DeserializedNode node) {
    ...
    for (uint32_t cnt = 0; cnt < N.StartDependOnMe.size(); cnt++) {
      ...
      int SIZEOF_NODE_INFO =
          sizeof(char) + sizeof(uint64_t) + sizeof(uint32_t) * 7 +
          sizeof(uint8_t) * 2 /*salvo: */ + sizeof(char) +
          sizeof(uint32_t) * 6 + sizeof(uint64_t) * 5 /* end salvo*/;
      uint32_t *dep_cnt = (uint32_t *)(mapping_start + sizeof(uint32_t) * 2 +
                                       sizeof(uint32_t) * num_root_nodes +
                                       SIZEOF_NODE_INFO * offset);
      (*dep_cnt)--;
      if ((*dep_cnt) == 0) {
    ...
    for (uint32_t cnt = 0; cnt < executableNodes.size(); cnt++) {
      ...
      */
      ...
      case OPTYPE_CALC:
        gp->type = OP_LOCOP;
        break;
      case OPTYPE_GEM5CALC:
        gp->type = OP_LOCGEM5OP;
        break;
      case OPTYPE_APPEND:
        gp->type = OP_APPEND;
        break;
...
public:
  ...
  Parser(char *filename, bool save_mem) {
    ...
    if (magic_cookie == MAGIC_COOKIE_INVALID) {
      fprintf(stderr, "This is serialized goal schedule was invalidated by a "
                      "prior simulation run!\n");
      exit(EXIT_FAILURE);
    }
    if (magic_cookie != MAGIC_COOKIE) {
      fprintf(stderr, "This is not a serialized goal schedule - the magic "
                      "cookie is missing\n");
      exit(EXIT_FAILURE);
    }
    ...
    if (save_mem == true) {
      // mmap can fail with map_private and prot_write on machines where the
      // virtual mem is smaller than
      // the mapped region - so we fall back to map_shared. This destroys the
      // schedule, so we invalidate
      // the magic cookie if we do this
      mapping_start = (char *)mmap(NULL, mapping_length, PROT_READ | PROT_WRITE,
      ...
      *((uint64_t *)mapping_start) = MAGIC_COOKIE_INVALID;
      ...
      // THIS NEEDS MORE MEMORY - but it is also more convinient for interacrive
      // use
      // because it preserves the schedules
      // Note that there is no fall-through to MAP_SHARED, we put the user in
      // charge now!
    }
    ...
    for (uint32_t cnt = 0; cnt < num_ranks_in_schedule; cnt++) {
      schedules.push_back(SerializedGraph(mapping_start + sizeof(uint64_t),
                                          mapping_length - sizeof(uint64_t),
                                          cnt));
    }
  }
  ...
  virtual uint32_t getHostCount() { return this->schedules.size(); }
  ...
  virtual void addRootNodes(ISimulator *sim) {
    ...
    int host = 0;
    int num_events = 0;
    for (Parser::schedules_t::iterator sched = this->schedules.begin();
         sched != this->schedules.end(); ++sched, ++host) {
...
// freeop->time = 0;
  ...
  virtual void addNewNodes(ISimulator *sim) {
    ...
    check_hosts.erase(unique(check_hosts.begin(), check_hosts.end()),
                      check_hosts.end());

    for (std::vector<int>::iterator iter = check_hosts.begin();
         iter != check_hosts.end(); ++iter) {
      host = *iter;
